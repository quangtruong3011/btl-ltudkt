...
class ManageOrder:
    ...
    def add_order(self, order):
        if not order.order_id or (
            order.order_id in [order.order_id for order in self.orders]
        ):
    ...
    def get_pending_orders(self):
        if (
            not self.orders
            and not self.display_orders_by_status(Order.STATUS_PENDING)
            and not self.display_orders_by_status(Order.STATUS_DELIVERING)
        ):
            print("Danh sách đơn hàng rỗng.")
            return []
